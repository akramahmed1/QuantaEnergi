...
# Skip liboqs import and use mock security by default
OQS_AVAILABLE = False
warnings.warn("liboqs disabled for MVP, using mock security")
...
try:
    ...
    df['production'] = pd.to_numeric(df['production'], errors='coerce').fillna(1000)
    real_data = df.iloc[0].to_dict()
except Exception as e:
    log_message(f"Error loading real data: {e}")
    real_data = {"price": 75.5, "production": 1000}
