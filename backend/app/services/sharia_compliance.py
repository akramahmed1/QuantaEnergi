"""
Consolidated Sharia Compliance Service for Islamic Finance Trading
Merged from: sharia_compliance.py and sharia.py
Handles AAOIFI compliance, commodity screening, Islamic trading rules, and validation
"""
...
class ShariaComplianceService:
    ...
    # ==================== ADDITIONAL VALIDATION METHODS (from sharia.py) ====================
    ...
    async def validate_transaction(self, trade: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            acceptable_gharar = 0.10  # 10% acceptable uncertainty
            if uncertainty > acceptable_gharar:
                raise HTTPException(status_code=400, detail=f"Excessive gharar (uncertainty): {uncertainty:.2%} > {acceptable_gharar:.2%}")
            
            # Check for haram assets
            haram_assets = ["alcohol", "pork", "gambling", "weapons", "tobacco", "pornography"]
            asset_type = trade.get("asset_type", "").lower()
            if asset_type in haram_assets:
    ...
    def validate_murabaha_contract(self, contract_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Dict with validation results
        """
        murabaha_markup_limit = 0.15  # 15% maximum markup for Murabaha
        markup = contract_data.get("markup_percentage", 0)
        
        if markup > murabaha_markup_limit:
            return {
                "compliant": False,
                "reason": f"Markup {markup:.2%} exceeds limit {murabaha_markup_limit:.2%}",
    ...
    def check_gharar_level(self, trade_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        gharar_threshold = 0.20  # 20% uncertainty threshold
        uncertainty = trade_data.get("uncertainty_level", 0)
        
        if uncertainty > gharar_threshold:
            return {
                "gharar_level": "high",
                "compliant": False,
                "reason": f"Uncertainty {uncertainty:.2%} exceeds threshold {gharar_threshold:.2%}",
                "screened_at": datetime.now().isoformat()
            }
